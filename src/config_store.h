...
typedef struct {
    char host[CONFIG_STR_CAP];
    char user[CONFIG_STR_CAP];
    char password[CONFIG_STR_CAP];
    int channel; /* defaults to 0 if absent/malformed */
} camera_config_t;
...
/* Reads $HOME/.config/calendar_pi/camera.conf ("host=...", "user=...",
 * "password=..." lines, "channel=..." optional, from your Reolink
 * camera's own settings -- see docs/REOLINK_SETUP.md). Returns 0 on
 * success, -1 if missing or malformed. */
